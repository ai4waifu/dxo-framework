...
impl Tensor {
    ...
    pub fn add(&self, other: &Tensor) -> Result<Tensor> {
        Ok(Tensor {
            inner: self.inner.add(&other.inner).map_err(map_err)?,
        })
    }
    ...
    pub fn matmul(&self, other: &Tensor) -> Result<Tensor> {
        Ok(Tensor {
            inner: self.inner.matmul(&other.inner).map_err(map_err)?,
        })
    }
    ...
    pub fn reshape(&self, shape: Vec<u32>) -> Result<Tensor> {
        ...
        Ok(Tensor {
            inner: self.inner.reshape(&shape).map_err(map_err)?,
        })
    }
...
pub fn tensor(data: Vec<f64>, shape: Vec<u32>) -> Result<Tensor> {
    ...
    let shape: Vec<usize> = shape.iter().map(|&d| d as usize).collect();
    Ok(Tensor {
        inner: CoreTensor::from_vec(data, shape).map_err(map_err)?,
    })
}
