...
const NATIVE_STUBS = [
    ...
    { name: '@dxo/dxo-darwin-arm64', os: ['darwin'], cpu: ['arm64'] },
    { name: '@dxo/dxo-darwin-x64', os: ['darwin'], cpu: ['x64'] },
];

const VERSION = '0.0.0';

const TRUST = {
    repo: 'ai4waifu/dxo-framework',
...
const STUBS = [
    ...
    ...NATIVE_STUBS.map((s) => ({
        ...s,
        description: `Optional native binary for @dxo/core (${s.name.replace(/^@dxo\//, '')}). Placeholder only.`,
    })),
];
...
function runNpm(args, opts = {}) {
    ...
    let userConfig;
    if (token) {
        userConfig = path.join(os.tmpdir(), `dxo-placeholder-npmrc-${process.pid}`);
...
function writeStub(spec, dir, authToken) {
    ...
    const pkg = {
        name: spec.name,
        version: VERSION,
        description: spec.description ?? 'DXO placeholder — not for production use.',
        license: 'MIT',
        private: false,
        files: ['README.md'],
    };
    ...
    fs.writeFileSync(path.join(dir, 'README.md'), `# ${spec.name}\n\nPlaceholder package (${VERSION}). Reserved for the DXO project.\n`);
    if (authToken) {
...
function cmdPublish() {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'dxo-npm-stubs-'));
