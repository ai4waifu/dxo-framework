 ...
 * - Contract: file=publish-npm.yml env=NPM_PUBLISH repo=ai4waifu/dxo-framework
 *
 * Prereq: native artifacts in DXO_NATIVE_ARTIFACTS (see publish-npm.yml).
 */
...
const JS_PACKAGES = [
    ...
    // Workspace-only stubs (hub/vision/llm/ui) are not published here.
];
...
function publishNative(version, artifactsRoot) {
    ...
    for (const plat of NATIVE_PLATFORMS) {
        ...
        const stage = path.join(os.tmpdir(), `dxo-pub-native-${plat.short}-${version}`);
        fs.rmSync(stage, { recursive: true, force: true });
        fs.mkdirSync(stage, { recursive: true });
        for (const f of fs.readdirSync(artDir)) {
            fs.copyFileSync(path.join(artDir, f), path.join(stage, f));
        }
        const want = `dxo.${plat.triple}.node`;
        writeJson(path.join(stage, 'package.json'), {
            name,
            version,
            description: `DXO native N-API addon (${plat.short})`,
        ...
        const plain = path.join(stage, 'dxo.node');
        ...
        fs.writeFileSync(
            ...
            `# ${name}\n\nOptional native binary for \`@dxo/core\` (${plat.short} / ${plat.triple}).\n`,
        );
        ...
        if (outcome === 'published') published += 1;
            ...
            fail(`OIDC/auth failed for ${name}. Add Trusted Publisher: file=publish-npm.yml env=NPM_PUBLISH repo=ai4waifu/dxo-framework`);
...
function publishJs(version) {
    let published = 0;
    let skipped = 0;
    const optionalNatives = Object.fromEntries(NATIVE_PLATFORMS.map((p) => [`@dxo/dxo-${p.short}`, version]));
    ...
    for (const spec of JS_PACKAGES) {
        ...
        const stage = path.join(os.tmpdir(), `dxo-pub-js-${name.replace(/[/@]/g, '-')}-${version}`);
        fs.rmSync(stage, { recursive: true, force: true });
        fs.mkdirSync(stage, { recursive: true });
...
const artifactsRoot = process.env.DXO_NATIVE_ARTIFACTS || path.join(ROOT, 'dist', 'native-flat');
