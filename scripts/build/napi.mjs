/**
 * Build `dxo-napi` and install the `.node` into the current-platform workspace package
 * (`projects/runtimes/dxo-<short>/` → npm name `@dxo/dxo-<short>`).
 *
...
const cargoArgs = ['build', '--manifest-path', path.join(root, 'Cargo.toml'), '-p', 'dxo-napi'];
...
const outDir = path.join(root, 'projects', 'runtimes', `dxo-${plat.short}`);
mkdirSync(outDir, { recursive: true });

const pkgJsonPath = path.join(outDir, 'package.json');
const binaryName = `dxo.${plat.triple}.node`;
if (!existsSync(pkgJsonPath)) {
    writeFileSync(
        pkgJsonPath,
        `${JSON.stringify(
            {
                name: `@dxo/dxo-${plat.short}`,
                version: '0.1.0',
                private: true,
                description: `DXO native N-API addon for @dxo/core (${plat.short})`,
    ...
    try {
        ...
        pkg.name = `@dxo/dxo-${plat.short}`;
        pkg.description = `DXO native N-API addon for @dxo/core (${plat.short})`;
        pkg.main = binaryName;
...
const destPlain = path.join(outDir, 'dxo.node');
...
console.log(`Platform package: @dxo/dxo-${plat.short} (${outDir})`);
