...
const linear = new FullyConnected(4, 2, { requiresGrad: false });
...
assert.ok(lg.nodes.some((n) => n.kind === 'FullyConnected'));

const seq = new Sequential([new FullyConnected(2, 3, { requiresGrad: false }), new ReLU(), new FullyConnected(3, 1, { requiresGrad: false })]);
